...
const Wishlist = () => {
  ...
  return (
    <Layout>
      ...
      <div className="bg-transparent min-h-screen py-12 px-4 md:px-8 font-body">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center gap-3 mb-8">
            <Heart className="w-8 h-8 text-primary fill-current" />
            <h1 className="text-4xl font-headline font-bold text-foreground">Your Wishlist</h1>
          </div>
          ...
          {items.length === 0 ? (
            <motion.div 
              ...
              className="glass-panel p-12 text-center border border-white/20 max-w-2xl mx-auto"
            >
              <Heart className="w-16 h-16 text-muted-foreground/40 mx-auto mb-6" />
              <h2 className="text-2xl font-headline font-bold mb-4 text-foreground">Your wishlist is empty</h2>
              <p className="text-muted-foreground mb-8">Save your favorite designs here to easily find them later when you're ready to start building your dream space.</p>
              <Button asChild className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-full px-8 py-6 text-base">
                <Link to="/designs">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
              {items.map((design) => {
                ...
                <motion.div 
                  ...
                  className="glass-card overflow-hidden group relative flex flex-col border border-white/20 transition-all duration-300 hover:-translate-y-1 hover:shadow-[var(--glass-shadow-lg)]"
                >
                  <Link to={linkTo} className="block relative aspect-[4/3] overflow-hidden bg-white/10 dark:bg-black/20">
                    <img 
                      src={design.image} 
                      alt={design.name} 
                      loading="lazy"
                  ...
                  <div className="p-5 flex flex-col flex-grow">
                    <div className="flex justify-between items-start mb-2">
                      <Link to={linkTo} className="hover:text-primary transition-colors">
                        <h3 className="font-headline font-bold text-xl line-clamp-1 text-foreground">{design.name}</h3>
                      </Link>
                    </div>
